       ...
       p4=np.array([n-1,m-1])   
        ...
        if np.sum(p4)==-2:
            if np.sum(xy_rver+xy_dhor)==0:
                ...
                if p4[1]<2*m/3:
                    p4[0]=m-1
    return cropped_img[p1[1]:p4[1],p1[0]:p4[0]]
    #cv2.rectangle(cropped_img,(p1[0],p1[1]), (p4[0],p4[1]),(0, 0, 255),5)


def board_detection():
    i=0
    for filename in glob.glob('./slides/slide_*.png'): #assuming png
    #for i in range (0, 7):
    #    img = cv2.imread("./slides/slide_" + str(i) + ".png")
        img = cv2.imread(filename)
        board_img=boardExtraction(img)
        cv2.imwrite("./slides/croppedSlide_" + str(i) + ".png", board_img)
        i=i+1
