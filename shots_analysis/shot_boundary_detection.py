...
def sbd(video_title, rect_points, t):
    ...
    with open(shots_list, 'w') as writer:
        while True:
            ...
            if sampled == MAX_SAMPLED:
                ...
                if max_sum + max_sum_diff > t:
                    if time - curr_shot.starting_time > 1.5:
                        ...
                        curr_shot = Shot(time, ' ')
                        # shot_properties = 'Shot ' + str(counter) +': ' + 'starting time: ' + str(starting_time) + ' ending time: ' + str(ending_time)
                        shot_data = {
                            "shot_number": str(counter),
                            "starting_time": str(round(starting_time, 2)),
                            "ending_time": str(round(ending_time, 2)),
                            "keywords": "yada yada yada"
                        }
                        json.dump(shot_data, writer)
