...
class DownloadWindow(QtWidgets.QMainWindow):
    ...
    def download(self):
        ...
        if lang == 'en':
            try:
                transcript = YouTubeTranscriptApi.get_transcripts(self.video_id, languages=['en'])
            except Exception as e:
                if not e:
    ...
    def analyze_video(self):
        ...
        print(isboard)
        if isboard > 0:
            slide_filtering(path)
        else:
            script = "python " + os.path.join('..', '..', 'main_script.py ') + path
